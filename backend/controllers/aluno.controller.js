...
endpointsFunction.getAllStudents = async (req, res) => {
  try {
...
endpointsFunction.getSubjectsEnrolledByStudent = async (req, res) => {
  const { id } = req.params;
  try {
    const dados = await Aluno.findOne({
      where: { id: id },
...
endpointsFunction.addSubjectToStudent = async (req, res) => {
  const { id } = req.params;
